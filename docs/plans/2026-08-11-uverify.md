...
### Task F2: Datenschutzerklärung & Impressum aktualisieren (Pflicht vor Go-Live)

**Files:**
- Modify: `src/pages/Privacy.tsx` (Datenschutzerklärung in der PWA)
- Modify: `src/pages/Impressum.tsx` (falls Verantwortlicher oder Kontakt sich ändert)

**Hintergrund:** Mit dieser Erweiterung ändert sich die Datenverarbeitung grundlegend gegenüber dem bisherigen Stand der Datenschutzerklärung. Folgende neue Verarbeitungstätigkeiten kommen hinzu:

| Neue Verarbeitungstätigkeit | Rechtsgrundlage | Verarbeiter |
|---|---|---|
| Cardano-Keypair wird im Browser erzeugt und in `localStorage` gespeichert | Vertragserfüllung (Art. 6 Abs. 1 lit. b DSGVO) | lokal, kein Dritter |
| Öffentlicher Schlüssel + Ed25519-Signatur werden an das AYM-Backend übermittelt zur Authentifizierung | Vertragserfüllung | AYM-eigener Server |
| Backend speichert: freigeschaltete Inhalte und abgeschlossene Episoden pro Profil (gekoppelt an Public Key) | Vertragserfüllung | AYM-eigener Server |
| MPF-Root (Hash der Profildaten) wird alle 48 h auf der Cardano-Blockchain verankert — dauerhaft öffentlich, nicht löschbar | Berechtigtes Interesse / Vertragserfüllung — **muss juristisch bewertet werden** | UVerify / Cardano |
| Stripe: Zahlungsabwicklung inkl. `client_reference_id` = `profileHash` (kein Name, aber pseudonymer Identifier) | Vertragserfüllung | Stripe Inc. (Auftragsverarbeiter, AVV erforderlich) |
| Zertifikats-URL enthält `pk`, `salt`, `profileId` als URL-Parameter — wird vom Nutzer geteilt | Nutzerentscheidung | UVerify UI (app.uverify.io) |

**Wichtige Hinweise:**
- Der MPF-Root ist ein Hash — kein personenbezogenes Datum im klassischen Sinne, aber dies muss vom zuständigen Datenschutzbeauftragten/Anwalt bewertet werden, da er dauerhaft und unwiderruflich auf einer öffentlichen Blockchain liegt.
- Für Stripe ist ein **Auftragsverarbeitungsvertrag (AVV)** mit Stripe Inc. abzuschließen (im Stripe-Dashboard verfügbar).
- UVerify (für die Zertifikatsseite) muss als Auftragsverarbeiter oder Dritter bewertet und ggf. in der Datenschutzerklärung genannt werden.
- Die 24-Wörter-Mnemonic verbleibt ausschließlich lokal — das ist datenschutzrechtlich positiv und sollte so kommuniziert werden.

**Steps:**

- [ ] **Step 1 (Mensch):** Rechtliche Prüfung: Sind MPF-Roots (Hashes von Nutzerdaten) personenbezogene Daten im Sinne der DSGVO? Ist eine Datenschutz-Folgenabschätzung (DSFA nach Art. 35 DSGVO) erforderlich? Juristischen Beistand hinzuziehen — dieser Schritt blockiert alle weiteren.
- [ ] **Step 2 (Mensch):** Auftragsverarbeitungsvertrag mit **Stripe** abschließen (Stripe-Dashboard → Settings → Legal → Data Processing Agreement).
- [ ] **Step 3 (Mensch):** Prüfen ob UVerify / app.uverify.io als Auftragsverarbeiter oder Dritter einzustufen ist — entsprechenden Vertrag oder Hinweis klären.
- [ ] **Step 4:** `src/pages/Privacy.tsx` aktualisieren: neue Abschnitte für Blockchain-Ankerpunkt, Stripe, UVerify, Keypair-Erzeugung im Browser, Hinweis auf Nicht-Löschbarkeit von Blockchain-Daten.
- [ ] **Step 5:** `src/pages/Impressum.tsx` prüfen — Verantwortlicher, Kontaktdaten, ggf. Datenschutzbeauftragter aktuell?
- [ ] **Step 6: Commit** — `legal: update privacy policy and imprint for blockchain identity + Stripe processing`

> ⚠️ **Dieser Task ist ein harter Go-Live-Blocker.** Kein produktiver Rollout, solange Step 1–3 nicht abgeschlossen und Step 4–5 nicht deployed sind.

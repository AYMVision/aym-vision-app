...
type ChapterStatus = 'completed' | 'current' | 'available' | 'locked';
...
function chapterStatus(
  ...
  if (chapter.chapterIndex0 <= highestPlayable) {
    return chapter.chapterIndex0 === highestPlayable ? 'current' : 'available';
  }
  return 'locked';
}
...
export default function StoryEpisodeOverview() {
  ...
  const highestPlayable = shouldBypassAll(courseId)
    ? (chapters?.length ?? 0) - 1
  ...
  const currentChapter = chapters?.find(
    (c) => chapterStatus(c, courseId, highestPlayable) === 'current'
  ) ?? chapters?.find(
    (c) => chapterStatus(c, courseId, highestPlayable) === 'available'
  ) ?? null;

  const allDone = completedCount >= totalCount;

  function handleAmicClick(chapter: StoryChapterV02) {
    const s = chapterStatus(chapter, courseId!, highestPlayable);
    if (s === 'locked') return;
        ...
        <section className="bg-white rounded-2xl border border-slate-100 shadow-sm divide-y divide-slate-100">
          ...
          {chapters === null ? (
            ...
            chapters.map((chapter) => {
              const status = chapterStatus(chapter, courseId, highestPlayable);
